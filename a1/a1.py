...
def bfs(graph, root, max_depth):
    ...
    while len(q)>0:
        ...
        for i in graph.neighbors(n):
            if i in seen and (node2distances[i] == node2distances[n]+1):
                    node2parents[i].append(n)
                    node2num_paths[i] += 1
            elif i not in seen and (node2distances[n] < max_depth):
                    q.append(i)
                    seen.add(i)           
                    node2parents[i].append(n)
                    node2num_paths[i] = 1
                    node2distances[i]=node2distances[n]+1 
    ...
    for k,v in node2distances.items():
        if k!=root:
            ns[k]=1.0
             ...
             ns[root]=0
    abc = sorted(node2distances.items(), key=lambda x: (-x[1]))
    for k,v in abc:
        if(node2num_paths[k]>1):
            n1=1/node2num_paths[k]
            for el in node2parents[k]:
                ns[el]=ns[el]+n1
                ns[k]=n1  
        elif(node2num_paths[k]==1):
            for i in node2parents[k]:
                ns[i]= ns[i]+ns[k]
    for k,v in ns.items(): 
        for i in node2parents[k]:
            result.append(k)
...
def approximate_betweenness(graph, max_depth):
    ...
    for root in graph.nodes():
        ...
        final = bottom_up(root, node2distances, node2num_paths, node2parents)
        for i in final:
            if i in result:
                result[i] = result[i]+final[i]
            else:
                result[i]= final[i]
...
def partition_girvan_newman(graph, max_depth):
    ...
    Returns:
      A list of networkx Graph objects, one per partition.
       ...
       ['D', 'E', 'F', 'G']
      """
    between = approximate_betweenness(graph, max_depth)
    cs = [m for m in nx.connected_component_subgraphs(graph)]
    gc = graph.copy()
    sd = sorted(between.items(), key = lambda x: (-x[1],x[0][0],x[0][1]))
    for i in sd:
        if(len(cs) > 1):
            break
        gc.remove_edge(*(i[0]))
        cs = [m for m in nx.connected_component_subgraphs(gc)]
        if len(cs) < 1:
                break
        elif len(cs) > 1:
                return cs    
...
def get_subgraph(graph, min_degree):
    ...
    n = graph.node()
    for k in n:
        if graph.degree(k) >= min_degree:
            graph_list.append(k)
    subgraph_formed = graph.subgraph(graph_list)
    return subgraph_formed
...
def norm_cut(S, T, graph):
    ...
    """
    vol_S = float(volume(S, graph))
    vol_T = float(volume(T, graph))
    ST = cut(S, T, graph)
    answer = (ST/vol_S) + (ST/vol_T)
    return answer
...
def brute_force_norm_cut(graph, max_size):
    ...
    for i in range(1, max_size+1):
        for j in combinations(graph.edges(), i):
            gph.remove_edges_from(j)
            components = get_components(gph)
            num = len(components)
            if num == 2:
                s = norm_cut(components[0], components[1], graph)
                edge_list = list(j)
                score.append((s, edge_list))
            gph.add_edges_from(j)
...
def score_max_depths(graph, max_depths):
    ...
    final=[]
    for r in max_depths:
        c=partition_girvan_newman(graph,r)
        c1=list(set(c[0].nodes()))
        c2=list(set(c[1].nodes()))
        u=norm_cut(c1,c2,graph)
        d=tuple((r,u))
        final.append(d)
    return final   
...
def make_training_graph(graph, test_node, n):
  ...
  To make a training graph, we need to remove n edges from the graph.
    ...
    answer=[]
    g=graph.copy()
    answer=sorted(g.edges(test_node))
    i=1
    for m in answer:
        if(i<=n):
            g.remove_edge(*m)
...
def jaccard(graph, node, k):
    ...
    for i in graph.nodes():
        if(i not in graph.neighbors(node) and i != node):
            answer2 = set(graph.neighbors(i))
            credits.append(((node, i), len(answer & answer2) /
                              len(answer | answer2)))
    return sorted(credits, key=lambda x: (-x[1],x[0]))[:k]
...
def evaluate(predicted_edges, graph):
    ...
    """
    total = 0
    for r in predicted_edges:
        if r[0] in graph.neighbors(r[1]):
            total += 1
