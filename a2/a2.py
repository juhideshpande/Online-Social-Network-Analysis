...
Original file is located at
    https://colab.research.google.com/drive/1YmMAYwNfxyXi2FS_AEJDyM_gVAtWhObT
"""
...
def cross_validation_accuracy(clf, X, labels, k):
    ...
    return np.mean(np.array(answer))
...
def mean_accuracy_per_setting(results):
    ...
    """
    dict_ans = defaultdict(lambda: 0)
    dict_count = defaultdict(lambda: 0)
    for setting in results:
        if setting['punct'] == True:
            dict_ans['punct=True'] = dict_ans['punct=True'] + setting['accuracy']
            dict_count['punct=True'] = dict_count['punct=True'] + 1
        else:
            dict_ans['punct=False'] = dict_ans['punct=False'] + setting['accuracy']
            dict_count['punct=False'] = dict_count['punct=False'] + 1
        dict_ans['min_freq=' + str(setting['min_freq'])] = dict_ans['min_freq=' + str(setting['min_freq'])] + setting['accuracy']
        dict_count['min_freq=' + str(setting['min_freq'])] =dict_count['min_freq=' + str(setting['min_freq'])] + 1
        finalFeatureName = 'feature='
        for featureName in setting['features']:
            finalFeatureName = finalFeatureName + str(featureName).split()[1] + ' '
        dict_ans[finalFeatureName] = dict_ans[finalFeatureName] + setting['accuracy']
        dict_count[finalFeatureName] = dict_count[finalFeatureName] + 1
    listOfAaccurracy = [(v, k) for k, v in dict_ans.items()]
    meanAccuracyList = []
    for item in listOfAaccurracy:
        meanAccuracy = item[0] / dict_count[item[1]]
        meanAccuracyList.append((meanAccuracy, item[1]))
    return sorted(meanAccuracyList, key =lambda tup: -tup[0])
